...
//   1. The LSP is spawned as bare `awkrs --lsp` (no `--stdio` appended) —
//      setting `transport: TransportKind.stdio` makes vscode-languageclient
//      append `--stdio`, which awkrs rejects ("connection got disposed" bug,
//      the same root cause that bit vscode-stryke).
//   2. A missing binary never constructs the LanguageClient (no retry/stop cascade).
//   3. Run + debug are registered, and the debug adapter is `awkrs --dap` at the
...
// A real executable on disk so resolveAwkBinary's accessSync(X_OK) passes.
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'awk-activate-'));
const fakeBin = path.join(tmp, 'awkrs');
...
test('LSP is spawned as bare `awkrs --lsp` (no --stdio-triggering transport)', () => {
...
test('missing binary → LanguageClient is never constructed', () => {
  loadExtensionWith(path.join(tmp, 'does-not-exist', 'awkrs'));
  assert.equal(clientCtorCalls, 0);
  assert.equal(captured, undefined);
});

test('run + debug commands and providers are registered (even with a missing binary)', () => {
  loadExtensionWith(path.join(tmp, 'nope', 'awkrs'));
  assert.equal(typeof reg.commands['awk.run'], 'function');
  assert.equal(typeof reg.commands['awk.debug'], 'function');
  assert.ok(reg.debugConfigProviders.awk, 'config provider registered for type awk');
  assert.ok(reg.debugAdapterFactories.awk, 'adapter factory registered for type awk');
});

test('debug adapter is `awkrs --dap` at the resolved absolute path', () => {
  loadExtensionWith(fakeBin);
  const desc = reg.debugAdapterFactories.awk.createDebugAdapterDescriptor({ configuration: {} });
  assert.ok(desc instanceof DebugAdapterExecutable);
  assert.equal(desc.command, fakeBin);
  assert.deepEqual(desc.args, ['--dap']);
});

test('per-session awkPath overrides the setting for the adapter', () => {
  const alt = path.join(tmp, 'awkrs-alt');
  fs.writeFileSync(alt, '#!/bin/sh\n'); fs.chmodSync(alt, 0o755);
  loadExtensionWith('awkrs-not-on-path');
  const desc = reg.debugAdapterFactories.awk.createDebugAdapterDescriptor({ configuration: { awkPath: alt } });
  assert.equal(desc.command, alt);
});

test('missing binary → adapter factory returns undefined (no broken session)', () => {
  loadExtensionWith(path.join(tmp, 'gone', 'awkrs'));
  const desc = reg.debugAdapterFactories.awk.createDebugAdapterDescriptor({ configuration: {} });
  assert.equal(desc, undefined);
});

test('F5 with no launch.json fills in the active .awk file as program', () => {
  loadExtensionWith(fakeBin, { document: { languageId: 'awk' } });
  const out = reg.debugConfigProviders.awk.resolveDebugConfiguration(undefined, {});
  assert.equal(out.type, 'awk');
